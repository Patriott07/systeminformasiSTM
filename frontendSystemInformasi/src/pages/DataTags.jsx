...
const DataTags = () => {
  ...
  const handleSubmitSearch = async (e) => {
    e.preventDefault();
    handleFetchTags(e.target[0].value);
  }
  ...
  const handleFetchTags = async (SQ) => {
    let url = `${serverPort}/tags/get?p=${currentPagination - 1}`;
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const res = await fetch(`${serverPort}/tags/delete/${id}`, {
            method: "DELETE",
            headers
          });
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex flex-grow">
        ...
        <div className="relative pb-8 overflow-x-auto shadow-md sm:rounded-lg w-full lg:w-9/12 m-10">
          ...
          <table className="text-sm text-left m-0 rounded-none rtl:text-right text-gray-500 dark:text-gray-400 border-t w-11/12 mx-auto">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th scope="col" className="px-9 py-3 text-end">
                  Action
                </th>
              </tr>
            </thead>
            <tbody>
              {tags.length > 0 ? tags.map((val, _i) => {
                return (
...
const EditModal = ({ data, onClose }) => {
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const res = await fetch(`${serverPort}/tags/update/${_id}`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
